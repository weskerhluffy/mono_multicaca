...
// XXX: https://www.hackerearth.com/practice/data-structures/trees/heapspriority-queues/practice-problems/algorithm/monk-and-multiplication/
#if 1
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
int hash_map_robin_hood_back_shift_borra(hm_rr_bs_tabla *ht, const void *key,
	...
	if (found) {
		...
		return 0;
		caca_log_debug("el num de buckes %u", ht->num_buckets_used_);
	}
	return 1;
}
...
heap_shit_nodo_llave *obten_llave(void *valor, heap_shit_nodo_llave *llave_res) {
	numero *a = valor;
	llave_res->contenido = &a->idx;
	llave_res->contenido_tam = sizeof(a->idx);

	caca_log_debug("regresando llave %u:%u", *(natural *)llave_res->contenido,
			llave_res->contenido_tam);
	return llave_res;
}
void *obten_prioridad(void *valor) {
	return NULL;
}
char *num_a_cad(void *valor, char *buffer) {
	numero *a = valor;
	sprintf(buffer, "%u:%u", a->num, a->idx);
	return buffer;
}

#define MAX_MIERDA ((int)1E5)

numero nums[MAX_MIERDA] = { 0 };
numero *nums_tmp[MAX_MIERDA] = { NULL };
...
static inline void caca_main() {
	...
	h = heap_shit_init(falso, compara_prioridad, obten_llave, obten_prioridad,
			num_a_cad);
	...
	for (natural i = 0; i < n; i++) {
		...
		heap_shit_insertar(h, na);

		if (i >= l - 1) {

			for (natural j = 0; j < l; j++) {
				numero *ma = heap_shit_borra_torpe(h);
				r *= ma->num;
				nums_tmp[j] = ma;
			}
			for (natural j = 0; j < l; j++) {
				heap_shit_insertar(h, nums_tmp[j]);
			}
		} else {
			r = -1;
		}
